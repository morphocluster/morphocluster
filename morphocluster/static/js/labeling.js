function init_tree() {
	...
	$(document).on("load_node.morphocluster", function (event, data) {
		...
		var processResponse = function (response, textStatus, jqXHR) {
			...
			$.each(response, function (k, member) {
				$row.append($('<div class="col col-2" />').append(render_member(member, ["star", "expand"])));
			});
	...
	function node_merge_into(node_id, dest_node_id) {
		...
		$("#node-status").text("Merging " + node_id + " into " + dest_node_id + "...");
	...
	$('#node-pane').on("click", ".action-favorite", function(event) {
		$this = $(this);
		console.log("Favourite", $this.closest(".node").data("node_id"));
		
		$this.removeClass("mdi-star-outline action-favorite mdi-inactive");
		$this.addClass("mdi-star action-unfavorite");
		
		return false;
	});
	
	$('#node-pane').on("click", ".action-unfavorite", function(event) {
		$this = $(this);
		console.log("Unfavorite", $this.closest(".node").data("node_id"));
		
		$this.removeClass("mdi-star action-unfavorite");
		$this.addClass("mdi-star-outline action-favorite mdi-inactive");
		
		return false;
	});
	
	$('#node-pane').on("click", ".action-move-up", function(event) {
		$this = $(this);
		console.log("Move up", $this.closest(".node").data("node_id"));
	...
	$('#node-pane').on("submit", "#node-userdata", function(event) {
		var node = appState.node;
		$("#node-status").text("Saving " + node.node_id + "...");
	...
	$('body').on("click", ".action", function(event) {
		var $this = $(this);
		var member_node_id = $this.closest(".member").data("node_id");
	...
	$("#node-pane").selectable({
		...
		stop: function () {
			...
			$("#node-status").text("Selected " + selectedMembers.length + " members.");
		}
	});
	...
	$umbrellaTermForm = $umbrellaTermDialog.find("form").on("submit", function(event) {
		...
		$.ajax({
			...
			$("#node-status").text("Merged " + selectedMembers.length + " members into " + response["node_id"]);
	...
	$("#btn-classify-members").on("click", function () {
		...
		$("#node-status").text("Classifying members of " + node_id + " into starred members...");
		...
		$.ajax({
			...
			$("#node-status").text(`Sorted ${resonse.n_predicted_children} children and ${resonse.n_predicted_objects} objects.`);
		}).fail(function (jqXHR, textStatus, errorThrown) {
			console.log(jqXHR, textStatus, errorThrown);
			$("#node-status").text(textStatus + ", " + errorThrown);
		});
