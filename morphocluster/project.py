...
def _rquery_subtree(node_id, recurse_cb=None):
    ...
    """
    q = select(
        [nodes, literal(0).label("level")]
    ).where(nodes.c.node_id == node_id).cte(recursive=True).alias("q")

    s = nodes.alias("s")

    rq = select([s, literal_column("level") + 1]
               ).where(s.c.parent_id == q.c.node_id)
...
class Project:
    ...
    def create(name, dataset_id) -> "Project":
        ...
        with connection.begin():
            ...
            stmt = (
                "CREATE TABLE nodes_objects_{project_id} PARTITION OF nodes_objects FOR VALUES IN ({project_id})"
            ...
            stmt = (
                "CREATE TABLE nodes_rejected_objects_{project_id} PARTITION OF nodes_rejected_objects FOR VALUES IN ({project_id})"
                .format(project_id=project_id)
            )
            result = connection.execute(stmt)
    ...
    def get_all(visible_only=True):
        ...
        qroots = select([nodes.c.project_id, nodes.c.node_id]
                       ).where(nodes.c.parent_id == None).alias("roots")
    ...
    def __init__(self, project_id):
        ...
        self._transaction: T.Optional[Transaction] = None
        self._locked = False
    ...
    def import_tree(self, tree):
        ...
        stmt = select(
            [func.count()]
        ).select_from(nodes).where(nodes.c.project_id == self.project_id)
        if self._connection.execute(stmt).scalar() != 0:
            raise RuntimeError(
                "Project project_id={} already contains a tree.".format(
                    self.project_id
                )
            )
        ...
        for node in tree.topological_order():
            ...
            parent_id = int(node["parent_id"]
                           ) if pd.notnull(node["parent_id"]) else None
    ...
    def create_node(
        ...
        **kwargs,
        ...
        if node_id is None:
            stmt = (
                select(func.max(nodes.c.node_id)
    ...
    def export_tree(self, tree_fn=None,
                    root_id=None) -> T.Optional[processing.Tree]:
        """
        ...
        keep_columns = [
            "orig_id", "parent_id", "name", 'starred', 'filled', 'approved',
            '_n_children', '_n_objects', '_n_objects_deep'
        ]
        ...
        node_objects = (
            select([nodes_objects.c.node_id, nodes_objects.c.object_id]
                  ).select_from(nodes_objects).where(
                      nodes_objects.c.node_id == subtree.c.node_id
                  )
        )
        ...
        node_rejected_objects = (
            ...
            ).select_from(nodes_rejected_objects).where(
                nodes_rejected_objects.c.node_id == subtree.c.node_id
            )
        )
    ...
    def get_root_id(self) -> T.Optional[int]:
        ...
        stmt = select(
            [nodes.c.node_id], (nodes.c.parent_id == None) &
    ...
    def to_dict(self) -> T.Dict:
        ...
        result = self._connection.execute(stmt,
                                          project_id=self.project_id).fetchone()
    ...
    def relocate_objects(self, object_ids, node_id, unapprove=False):
        ...
        # TODO: Distinct!
        stmt = (
            select([nodes_objects.c.node_id], for_update=True).where(
    ...
    def merge_node_into(self, node_id, dest_node_id, unapprove=False):
        ...
        stmt = (
            # pylint: disable=no-value-for-parameter
            nodes_objects.update().values(node_id=dest_node_id).where(
        ...
        stmt = (
            # pylint: disable=no-value-for-parameter
            nodes.update().values(parent_id=dest_node_id).where(
                nodes.c.parent_id == node_id
                & nodes.c.project_id == self.project_id
            )
        )
        self._connection.execute(stmt)
    ...
    def invalidate_nodes(self, nodes_to_invalidate, unapprove=False):
        ...
        stmt = (
            # pylint: disable=no-value-for-parameter
            nodes.update().values(values).where(
                nodes.c.node_id.in_(nodes_to_invalidate)
            )
        )
        self._connection.execute(stmt)
    ...
    def get_children(
        self, node_id, require_valid=True, order_by=None, include=None
    ):
        """
        Parameters:
    ...
    def get_objects(self, node_id, offset=None, limit=None, order_by=None):
        ...
        stmt = select([objects]).select_from(
            objects.join(nodes_objects)
    ...
    def calculate_progress(self, node_id=None):
        ...
        subtree["n_filled_objects"] = subtree["filled"] * \
            subtree["_n_objects_deep"]
        subtree["n_named_objects"] = pd.notna(subtree["name"]
                                             ) * subtree["_n_objects_deep"]
        ...
        fields = [
            "_n_objects", "_n_objects_deep", "n_filled_objects",
            "n_approved_objects", "n_named_objects", "n_approved_nodes",
            "n_filled_nodes", "n_nodes"
        ]
        ...
        leaves_result = {
            "leaves_{}".format(k.lstrip('_')): v
            for k, v in leaves_result.items()
        }
    ...
    def update_node(self, node_id, data):
        ...
        stmt = (
            nodes  # pylint: disable=no-value-for-parameter
            .update().values(data).where(
                nodes.c.node_id == node_id
                & nodes.c.project_id == self.project_id
            )
        )
    ...
    def recommend_children(self, node_id, max_n=1000):
        ...
        for parent_id in path[:-1][::-1]:
            ...
            nodes_.extend(
                c for c in self.get_children(parent_id)
                if c["node_id"] not in path
            )
    ...
    def recommend_objects(self, node_id, max_n=1000):
        ...
        with Timer("Tree.recommend_objects") as timer:
            ...
            rejected_object_ids = (
                select([nodes_rejected_objects.c.object_id]).where(
    ...
    def get_next_node(
        ...
        if node["parent_id"]:
            print(
                "No unapproved children, trying parent: {}".format(
                    node["parent_id"]
                )
            )
            return self.get_next_node(
                node["parent_id"], leaf, recurse_cb, filter
            )
    ...
    def __exit__(self, type_, value, traceback):
        if type_ is None and self._transaction is not None:
    ...
    def remove(self):
        ...
        with connection.begin():
            ...
            stmt = (
                "DROP TABLE nodes_objects_{project_id} CASCADE"
            ...
            stmt = (
                "DROP TABLE nodes_rejected_objects_{project_id} CASCADE"
                .format(project_id=self.project_id)
            )
            connection.execute(stmt)
    ...
    def consolidate_node(self, node_id, depth=0, descend_approved=True, return_=None):
        ...
        with connection.begin():
            ...
            if not invalid_subtree["cache_valid"].all():
                ...
                invalid_subtree["_n_objects_deep"] = invalid_subtree["_n_objects_deep"].astype(
                    int)
